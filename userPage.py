...
def user_page(username):
    ...
    with stylable_container(
        ...
        with col1:
            reason_button = st.button("💡 Reason", key="toggle_thinking", type=thinking_type, help="Enable reasoning mode for more advanced answers")
            if reason_button:
                st.session_state.thinking = not st.session_state.thinking
                # Turn off image mode if reasoning is activated
                if st.session_state.thinking:
                    st.session_state.image = False
                st.rerun()
        
        with col2:
            # Check if there's an uploaded file and disable search button accordingly
            file_uploaded = False
            if prompt and prompt.get('files'):
                file_uploaded = True
            
            search_button = st.button("🔍 Search", key="toggle_search", type=search_type, disabled=file_uploaded, help="Enable web search for your queries")
            if search_button:
                st.session_state.search = not st.session_state.search
                # Turn off image mode if search is activated
                if st.session_state.search:
                    st.session_state.image = False
                st.rerun()
        
        with col3:
            image_button = st.button("📷 Image", key="toggle_image", type=image_type, disabled=st.session_state.thinking or st.session_state.search, help="Generate an image based on your prompt")
            if image_button:
